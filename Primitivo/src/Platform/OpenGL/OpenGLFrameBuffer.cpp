...
namespace Primitivo {
	...
	namespace Utils {
		...
		static GLenum PrimitivoFrameBufferTextureFormatToOpenGL(FrameBufferTextureFormat format) {
			switch (format) {
	...
	void OpenGLFrameBuffer::ClearColorAttachment(uint32_t attachmentIndex, const void* value) {
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::PrimitivoFrameBufferTextureFormatToOpenGL(specification.TextureFormat), GL_INT, &value);
	}
}