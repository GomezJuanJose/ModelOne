...
namespace ImGui
{
    ...
    IMGUI_API ImDrawList*   GetWindowDrawList();                        // get draw list associated to the current window, to append your own drawing ModelOnes
    ...
    IMGUI_API void          EndGroup();                                                     // unlock horizontal starting position + capture the whole group bounding box into one "item" (so you can use IsItemHovered() or layout ModelOnes such as SameLine() on whole group, etc.)
...
// access the current window draw list and draw custom ModelOnes.
// You can interleave normal ImGui:: calls and adding ModelOnes to the current draw list.
// In single viewport mode, top-left is == GetMainViewport()->Pos (generally 0,0), bottom-right is == GetMainViewport()->Pos+Size (generally io.DisplaySize).
// You are totally free to apply whatever transformation matrix to want to the data (depending on the use of the transformation you may want to apply it to ClipRect as well!)
// Important: ModelOnes are always added to the list and not culled (culling is done at higher-level by ImGui:: functions), if you use this API a lot consider coarse culling your drawn objects.
struct ImDrawList
{
    ...
    ImDrawListFlags         Flags;              // Flags, you may poke into these to adjust anti-aliasing settings per-ModelOne.
    ...
    // - For rectangular ModelOnes, "p_min" and "p_max" represent the upper-left and lower-right corners.
    // - For circle ModelOnes, use "num_segments == 0" to automatically calculate tessellation (preferred).
    ...
    IMGUI_API void  AddDrawCmd();                                               // This is useful if you need to forcefully create a new draw call (to allow for dependent rendering / blending). Otherwise ModelOnes are merged into the same draw-call as much as possible
    IMGUI_API ImDrawList* CloneOutput() const;                                  // Create a clone of the CmdBuffer/IdxBuffer/VtxBuffer.

    // Advanced: Channels
    // - Use to split render into layers. By switching channels to can render out-of-order (e.g. submit FG ModelOnes before BG ModelOnes)
    ...
    // Advanced: ModelOnes allocations
    // - We render triangles (three vertices)
    // - All ModelOnes needs to be reserved via PrimReserve() beforehand.
