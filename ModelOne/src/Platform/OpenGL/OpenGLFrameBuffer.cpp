...
namespace ModelOne {
	...
	namespace Utils {
		...
		static GLenum ModelOneFrameBufferTextureFormatToOpenGL(FrameBufferTextureFormat format) {
			switch (format) {
			case FrameBufferTextureFormat::RGBA8: return GL_RGBA8;
			case FrameBufferTextureFormat::DEPTH_COMPONENT: return GL_DEPTH_COMPONENT;
			}

			MO_ASSERT(false, "Format not found!");
			return 0;
		}
	}
	...
	void OpenGLFrameBuffer::Invalidate() {
		...
		if (m_ColorAttachments.size() > 1) {
			MO_ASSERT(m_ColorAttachments.size() <= 4, "At the moment only supports a maximum of 4 color buffers");
	...
	void OpenGLFrameBuffer::Resize(uint32_t width, uint32_t height) {
		if (width == 0 || height == 0 || width > s_MaxFrameBufferSize || height > s_MaxFrameBufferSize) {
			MO_LOG_WARNING(true, "Attempted to resize framebuffer to %i %i", width, height);
			return;
		}
	...
	void OpenGLFrameBuffer::ClearColorAttachment(uint32_t attachmentIndex, const void* value) {
		MO_ASSERT(attachmentIndex < m_ColorAttachments.size(), "Index out of bounds!");

		auto& specification = m_ColorAttachmentSpecifications[attachmentIndex];

		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::ModelOneFrameBufferTextureFormatToOpenGL(specification.TextureFormat), GL_INT, &value);
	}
}